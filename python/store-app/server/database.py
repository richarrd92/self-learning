...
from sqlalchemy import create_engine, text  # Database engine and raw SQL execution # type: ignore 
from sqlalchemy.ext.declarative import declarative_base  # ORM base class # type: ignore 
from sqlalchemy.orm import sessionmaker  # Session management # type: ignore 
from dotenv import load_dotenv  # Load environment variables from .env file # type: ignore 
...
# The engine manages the connection pool and interacts with the database
engine = create_engine(DATABASE_URL)

# Create a session factory for handling database transactions
# - `autocommit=False`: Changes require explicit commits
# - `autoflush=False`: Prevents automatic flushing (writing) of pending changes
