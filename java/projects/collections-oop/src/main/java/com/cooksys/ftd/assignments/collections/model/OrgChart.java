...
public class OrgChart {
    ...
    private Set<Employee> orgChart = new HashSet<>(); // store unique employees
    ...
    public boolean addEmployee(Employee employee) {
        // Check if the employee is null or already in the org chart
        if (employee == null || hasEmployee(employee)) return false;

        // Standalone worker with no manager cannot be added
        if (!employee.hasManager() && !(employee instanceof Manager)) return false;

        // Add all the employee's managers in the chain (direct manager first)
        for (Manager manager : employee.getChainOfCommand()) {
            // only add new managers
...
//        Set<Employee> copyOfOrgChart = new HashSet<>();
//        for(Employee employee : orgChart){
//            copyOfOrgChart.add(employee);
//        }
//        return copyOfOrgChart;
    ...
    public Set<Manager> getAllManagers() {
        Set<Manager> copyOfOrgChartManagers = new HashSet<>();
        for(Employee employee : orgChart){
            // only add managers
            if (employee instanceof Manager){
                // convert employee to manager
                copyOfOrgChartManagers.add((Manager) employee);
            }
        }

        // copy of orgChart but only managers
        return copyOfOrgChartManagers;
    }
    ...
    public Map<Manager, Set<Employee>> getFullHierarchy() {
        Map<Manager, Set<Employee>> orgChartFullHierarchy = new HashMap<>();
        // Get all managers -> Keys
        for(Manager manager : getAllManagers()){
            // Get all subordinates of each manager as a set -> Values
            Set<Employee> managerSubordinates = new HashSet<>();
            managerSubordinates = getDirectSubordinates(manager);

            // store in the orgChartFullHierarchy map
            orgChartFullHierarchy.put(manager, managerSubordinates);
        }
        return orgChartFullHierarchy;
    }
